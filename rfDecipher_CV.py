...
for n in range(nrepeat):  
    ...
    bestparams[n,1]= best_n_estimators
    ...
    for train_index, test_index in skf.split(gexp, labels):
        ...
        rfc=RandomForestClassifier(max_depth=best_max_depth,
...
filename='rf_decipher_aprstat.csv'
...
filename='rf_decipher_cmatrix.csv'
with open(filename, 'w', newline="") as file:
    csvwriter = csv.writer(file)
    csvwriter.writerows(cmatrix)
    
filename='rf_decipher_best_params.csv'
with open(filename, 'w', newline="") as file:
    csvwriter = csv.writer(file)    
